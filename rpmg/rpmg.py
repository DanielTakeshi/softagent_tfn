...
class simple_RPMG(torch.autograd.Function):
    ...
    '''
    @staticmethod
    def forward(ctx, in_nd, tau, lam, weight=1, rgt=None):
        proj_kind = in_nd.shape[1]
        if proj_kind == 6:
            r0 = tools.compute_rotation_matrix_from_ortho6d(in_nd)
        elif proj_kind == 9:
        ...
        if rgt is None:
            rgt = torch.zeros_like(r0)

        ctx.save_for_backward(in_nd, r0, torch.Tensor([tau,lam, weight]), rgt)
        # return r0.transpose(-1, -2)  # TODO(daniel): getting rid of this?
        return r0

    @staticmethod
    def backward(ctx, grad_in):
        # grad_in = grad_in.transpose(-1, -2) # TODO(daniel): getting rid of this?
        in_nd, r0, config, rgt = ctx.saved_tensors
        tau = config[0]
        lam = config[1]
        weight = config[2]
        ...
        if tau == -1:
            # use tau_gt if tau = -1
            # requires rgt to be passed in initially
            r_new = rgt
        else:
            # Eucliean gradient -> Riemannian gradient
            Jx = torch.zeros((b, 3, 3)).cuda()
